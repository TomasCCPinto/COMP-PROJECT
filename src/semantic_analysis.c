...
static void add_params(ast_node_t *node, param_list **global_head, param_list **func_head, symbol_table **symbol_head) {
    if (!node)
	return;
    ...
    if (!(*global_head) && !(*func_head) && !(*symbol_head)) {
        ...
        *symbol_head = symbol_table_node(node->child->brother->value, node->child->id, true, false);
	    add_params(node->brother, global_head, func_head, &(*symbol_head)->symbols);
	    // add_params(node->brother, &(*global_head)->next, &(*func_head)->next, symbol_head);
	    return;
    } 
    if (global_head && func_head && symbol_head) {
        add_params(node, &(*global_head)->next, &(*func_head)->next, symbol_head);
    } else if (global_head) {
        add_params(node, &(*global_head)->next, func_head, symbol_head);
    } else if (func_head) {
        add_params(node, global_head, &(*func_head)->next, symbol_head);
    }
}

static void add_body_params(ast_node_t *node, symbol_table **symbol_node) {
    if (!node)
        return;
    ...
    if (!(*symbol_node)) {
        ...
        add_body_params(node->brother, symbol_node);
        return;
    }
    if (*symbol_node)
        add_body_params(node, &(*symbol_node)->symbols);
}
...
static void append_symbol_table(ast_node_t *node, symbol_table *symbol_node) {
    ...
    if (node->child->brother->brother && node->child->brother->brother->child && !strcmp(node->child->brother->brother->child->id, "ParamDecl")) {
	    add_params(node->child->brother->brother->child, &symbol_node->symbols->params, &symbol_node->next->params, &symbol_node->next->symbols->symbols);
    }
    if (node->brother && !strcmp(node->brother->id, "MethodBody")) {
        add_body_params(node->brother->child, &symbol_node->next);
    }
}
